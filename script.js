    ...
    .forEach(p=>{
      l.innerHTML+=`
        <div class="produto">
          ...
          <button class="btn-add" onclick="add(${p.id})">+</button>
        </div>
      `;
    });
}
...
function add(id){
  ...
  const e=carrinho.find(x=>x.id===id);
...
function setTipo(t){
  ...
  document.getElementById('btn-entrega')
    .classList.toggle('ativo-tipo',t==='entrega');

  document.getElementById('btn-retirada')
    .classList.toggle('ativo-tipo',t==='retirada');
...
function upd(){
  ...
  let fr=(tipoEntrega==='entrega'&&etapa>=2)?FRETE:0;
  let tot=s+fr;

  document.getElementById('total-bar').innerText=
    `R$ ${s.toFixed(2).replace('.',',')} • ${q} itens`;

  document.getElementById('subtotal-modal').innerText=
    `R$ ${s.toFixed(2).replace('.',',')}`;

  document.getElementById('total-modal').innerText=
    `R$ ${tot.toFixed(2).replace('.',',')}`;

  document.getElementById('frete-modal').innerText=
    (tipoEntrega==='entrega'&&etapa>=2)
      ?`R$ ${FRETE.toFixed(2).replace('.',',')}`
      :'A calcular';

  const d=document.getElementById('itens-carrinho');
  d.innerHTML='';
      ...
      <div class="item-car">
        ...
        <div style="display:flex;gap:8px;align-items:center;">
          <button onclick="removerCarrinho(${p.id})"
            style="width:28px;height:28px;border-radius:50%;background:#222;color:#fff;border:1px solid #333;">
            -
          </button>
          ...
          <button onclick="add(${p.id})"
            style="width:28px;height:28px;border-radius:50%;background:#FFD700;border:none;">
            +
          </button>
        </div>
...
function abrirCarrinho(){
  if(!estaAberto())
    return alert('FECHADO - Abre 09h-14h30 e 19h-00h30');
...
function mostrarEtapa(){
  document.getElementById('etapa-1').style.display=
    etapa===1?'block':'none';

  document.getElementById('etapa-2').style.display=
    etapa===2?'block':'none';

  document.getElementById('etapa-3').style.display=
    etapa===3?'block':'none';

  document.getElementById('btn-voltar-etapa').style.display=
    etapa===1?'none':'block';

  document.getElementById('titulo-etapa').innerText=
    etapa===1?'1. Seus dados':
    etapa===2?'2. Endereço':
    '3. Pagamento';

  document.getElementById('btn-proxima').innerText=
    etapa===3?'ENVIAR NO ZAP':'CONTINUAR';
...
function voltarEtapa(){
  ...
  if(etapa===3&&tipoEntrega==='retirada')
    etapa=1;
...
function mostrarPix(){
  ...
  const a=document.getElementById('area-pix');

  if(p==='Pix'){

    a.innerHTML=`
      <p style="color:#FFD700;font-size:11px;font-weight:900;">
        CHAVE PIX CPF:
      </p>
      ...
      <div style="display:flex;gap:8px;margin-top:8px;">
        ...
        <button
          onclick="navigator.clipboard.writeText('${CHAVE_PIX}');alert('Copiado')"
          style="width:80px;background:#FFD700;color:#000;font-weight:900;border:none;border-radius:8px;">
          COPIAR
        </button>
    ...
    a.innerHTML=`
      <input id="troco" placeholder="Troco pra quanto?">
    `;
...
function enviarZap(){
  ...
  let fr=tipoEntrega==='entrega'?FRETE:0;
  let tot=s+fr;

  let txt=`*PEDIDO FABRICA DE SABORES*%0A`;

  carrinho.forEach(p=>{
    txt+=`${p.qtd}x ${p.nome} - R$ ${p.preco}%0A`;
  });

  txt+=`
%0ASubtotal R$ ${s} + Frete R$ ${fr} = Total R$ ${tot}
%0ACliente ${n}
%0ATipo ${tipoEntrega}
%0APix ${CHAVE_PIX}`;

  window.open(
    `https://wa.me/${ZAP}?text=${txt}`,
    '_blank'
  );
}
